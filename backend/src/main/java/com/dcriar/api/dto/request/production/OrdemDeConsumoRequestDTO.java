...
public class OrdemDeConsumoRequestDTO {
    ...
    @Schema(description = "ID do produto a ser fabricado (deve ser um produto de consumo).", example = "13", requiredMode = Schema.RequiredMode.REQUIRED)
    private Long produtoId;

    @Schema(description = "ID do lote de matéria-prima a ser consumido.", example = "11", requiredMode = Schema.RequiredMode.REQUIRED)
    private Long loteId;

    @Schema(description = "ID do canal de venda de destino do estoque (opcional).", example = "1")
    private Long canalVendaDestinoId;

    @Schema(description = "Quantidade de unidades do produto a serem produzidas.", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
    private Integer quantidadeProduzida;

    @Schema(description = "Motivo ou referência para a ordem.", example = "Reposição de estoque de verniz para Loja Física")
