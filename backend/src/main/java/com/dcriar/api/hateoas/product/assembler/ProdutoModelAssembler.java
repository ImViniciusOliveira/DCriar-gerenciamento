...
public class ProdutoModelAssembler extends RepresentationModelAssemblerSupport<ProdutoResponseDTO, ProdutoModel> {
    ...
    public ProdutoModel toModel(@NonNull ProdutoResponseDTO dto) {
        ...
        if (dto.getId() != null) {
            ...
            // Ajuste: uploadFoto agora tem 3 parâmetros (id, MultipartFile, HttpServletRequest)
            model.add(linkTo(methodOn(ProdutoController.class).uploadFoto(dto.getId(), null, null)).withRel("upload-foto"));
